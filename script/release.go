...
func main() {
	...
	bumpingVersionMessage := fmt.Sprintf("Bumping FrequentFlyer version to %s", newVersion)
	...
	didBumpVersionMessage := fmt.Sprintf("Finished setting FrequentFlyer version to %s", newVersion)
	...
	successMessage := fmt.Sprintf("Finished successfully updating FrequentFlyer to version %s", newVersion)
