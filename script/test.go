...
func main() {
	...
	testInitReport := fmt.Sprintf("Running FrequentFlyer unit tests with iOS version '%s' on  device type '%s'\n", iosVersion, deviceVersion)
	...
	unitTestCommand := exec.Command("xcodebuild", "test", "-workspace", "FrequentFlyer.xcworkspace", "-scheme", "FrequentFlyer", "-destination", destinationString)
	...
	_, err = stdOut.Write([]byte("FrequentFlyer unit tests passed...\n"))
	if err != nil {
		log.Fatal(err)
	}
}
